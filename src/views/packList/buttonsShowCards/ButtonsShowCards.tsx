...
export const ButtonsShowCards = () => {
    ...
    return (
        <div className={style.container}>
            <button className={packId ? style.button : `${style.button} ${style.active}`} onClick={showAllCards}>All
            </button>
            <button className={!packId ? style.button : `${style.button} ${style.active}`} onClick={showMyCards}>My
            </button>
        </div>
    );
};